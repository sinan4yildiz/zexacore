<template>
  <button @click="$emit('click')" :type="type || 'button'" v-bind:class="buttonClasses" v-bind:disabled="loading">
    <span v-if="loading" v-bind:class="{'-ml-1.5 mr-2': label}" class="inline-block w-5 h-5 text-center">
      <svg class="w-3.5 h-3.5 animate-spin animate-spin-fast">
...
export default {
  ...
  computed: {
    ...
    buttonClasses: function () {
      let b = ['flex align-items-center text-sm font-medium rounded-md whitespace-no-wrap focus:outline-none transition duration-150 ease-in-out select-none']
