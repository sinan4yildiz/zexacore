  ...
  <section>
    <header class="md:flex md:items-center mb-5">
      ...
      <div class="mb-4 md:mb-0">
        <h1 class="mb-2 text-2xl lg:text-3xl leading-8 lg:leading-9 text-gray-800 truncate">{{ $t('articles.heading.edit') }}</h1>
    ...
    <article v-if="article.data" class="grid grid-cols-12 gap-6">
      ...
      <div class="overflow-hidden col-span-full lg:col-span-8 xl:col-span-9 bg-white rounded-lg shadow">
        ...
        <!-- Language picker -->
        <LanguagePicker :changeable="true" :translations="article.data.translations" :current="article.data.language_code" :routed="true" @input="changeLanguage($event)"/>
        ...
        <ul>
          ...
          <li class="xl:grid xl:grid-cols-3 xl:gap-4 items-center py-4 px-4 xl:px-6 bg-gray-50 border-b">
            ...
            <div class="xl:col-span-2 mt-2 xl:mt-0 mb-1 xl:mb-0">
              <MultiSelect :selecteds="selectedCategories(article.data.categories)"
                           :params="{content_type_id: article.data.content_type_id}"
                           :placeholder="$t('placeholder.search_categories')"
                           name="categories"
                           action="Categories/categoryAutocomplete"
                           @input="form.categories = $event"
                           :errors="errors"/>
            </div>
          </li>
      ...
      <div class="col-span-full lg:col-span-4 xl:col-span-3">
        <ul class="overflow-hidden bg-gray-50 rounded-lg shadow">
          ...
          <li v-if="article.data.language_code != settings.default_language_code" class="items-center py-4 px-4 xl:px-6 border-b">
            <div>
            ...
            <div class="mt-2 mb-1">
              <Autocomplete name="translation_id"
                            :value="article.data.translation ? article.data.translation.id : null"
                            :keyword="article.data.translation ? article.data.translation.title : null"
                            :placeholder="$t('placeholder.search_articles')"
                            action="Articles/articleAutocomplete"
...
export default {
  ...
  methods: {
    ...
    selectedCategories(categories) {
      return _.map(categories, (c) => ({ value: c.category_id, text: c.title }));
    },
