  ...
  <div v-on-clickaway="close" class="field w-full relative" v-bind:class="classes">
    ...
    <div class="relative">
      <input v-model="inputKeyword"
             ...
             class="form-input block w-full px-4 py-3 text-sm border border-gray-400 focus:border-blue-400 focus:shadow-outline-blue rounded-md shadow-sm transition duration-150 ease-in-out pr-12"
             autocomplete="off">
      <input v-model="inputValue" type="hidden" :name="name">
      <label v-if="inputValue" @click="clear" :for="name" class="flex items-center absolute top-0 right-0 bottom-0 px-4 py-3 text-gray-500 cursor-pointer hover:text-gray-700">
        <svg class="w-5 h-5 transition duration-300 ease-out">
          <use xlink:href="#icon-backspace"></use>
        </svg>
      </label>
      <label v-else :for="name" class="flex items-center absolute top-0 right-0 bottom-0 px-4 py-3 text-gray-400">
        <svg class="w-5 h-5 transition duration-300 ease-out">
