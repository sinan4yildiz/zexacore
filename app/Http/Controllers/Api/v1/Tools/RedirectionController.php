...
class RedirectionController extends Controller
{
    ...
    public function create(RedirectionRequest $request)
    {
        ...
        $activity->description = trans('admin/activitiy.redirection.create', [
            'url' => request('from'),
        ]);
        $activity->save();
    ...
    public function update(RedirectionRequest $request, $id)
    {
        ...
        $activity = new Activity;
        $activity->user = Auth::user()->firstname . ' ' . Auth::user()->lastname;
        $activity->description = trans('admin/activitiy.redirection.update', [
            'url' => request('from'),
        ]);
        $activity->save();
    ...
    public function remove($id)
    {
        ...
        $activity = new Activity;
        $activity->user = Auth::user()->firstname . ' ' . Auth::user()->lastname;
        $activity->description = trans('admin/activitiy.redirection.remove', [
            'url' => request('from'),
        ]);
        $activity->save();
